...
@Slf4j
@Service
public class DeptTreeCrudService {
    @Autowired
     ...
     * 获取某个节点的子树节点
     */
    public List<DeptEntity> getSubTreeList(Long id) {
        if(id == null || id < 1) {
            return null;
        }
    ...
    public List<DeptVo> getChildren(Long pid) {
        ...
        return BeanCopyUtils.copyList(deptEntityList, DeptVo.class);
    }
     ...
     * 获取全部叶子节点
     */
    public List<DeptVo> getLeafNode() {
        List<DeptEntity> leftNodeList1 = deptMapper.getLeafNodeByExists();
        System.out.println("leftNodeList1 = " + JSON.toJSONString(leftNodeList1));

        List<DeptEntity> leftNodeList2 = deptMapper.getLeafNodeByLeftJoin();
        System.out.println("leftNodeList2 = " + JSON.toJSONString(leftNodeList2));

        return BeanCopyUtils.copyList(leftNodeList1, DeptVo.class);
    }

    /**
     * 获取某个节点子树的叶子节点
     */
    public List<DeptVo> getLeafNode(long id) {
        List<DeptEntity> deptEntityList = deptMapper.getSubTreeLeftNode(id);

        return BeanCopyUtils.copyList(deptEntityList, DeptVo.class);
    }

    public List<DeptVo> getLevelNode(int level) {
        List<DeptEntity> deptEntityList = deptMapper.getLevelNode(level);

        return BeanCopyUtils.copyList(deptEntityList, DeptVo.class);
    }

    public List<DeptVo> getLevelNode(long id, int level) {
        List<DeptEntity> deptEntityList = deptMapper.getSubTreeLevelNode(id, level);
        return BeanCopyUtils.copyList(deptEntityList, DeptVo.class);
    }
     ...
     * 获取某个节点的父路径
     */
    public String getParentFullPath(long id) {
        DeptEntity deptEntity = deptMapper.getParentFullPath(id);
        System.out.println(JSON.toJSONString(deptEntity));

        DeptEntity dept = deptMapper.selectById(id);
        if(dept == null) {
            throw new IllegalArgumentException("节点不存在");
        }
        String path = dept.getAncestorPath() + "/" + dept.getId();
        String[] split = path.split("/");
        Set<String> ids = Arrays.stream(split).collect(Collectors.toSet());
        List<DeptEntity> deptEntities = deptMapper.selectList(Wrappers.<DeptEntity>lambdaQuery().in(DeptEntity::getId, ids));
        Map<Long, DeptEntity> deptMap = deptEntities.stream().collect(Collectors.toMap(DeptEntity::getId, obj -> obj));
        String name = "";
        for (String s : split) {
            if(StringUtils.isBlank(s)) {
                continue;
            }
            name += "/" + deptMap.get(Long.parseLong(s)).getName();
        }
        return name;
    }
